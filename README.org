...
#+BEGIN_SRC clojure
  ...
  (let [chimes (chime-ch [(.plusSeconds (java.time.Instant/now) 2)
                          (.plusSeconds (java.time.Instant/now) 3)])]
    (a/<!! (go-loop []
             (when-let [msg (<! chimes)]
               (prn "Chiming at:" msg)
               (recur)))))
...
#+BEGIN_SRC clojure
  (defn periodic-seq [^java.time.Instant start duration-or-period]
      (iterate
        #(.addTo duration-or-period %)
        start))

  (rest    ; excludes *right now*
   (periodic-seq (java.time.Instant/now)
...
#+BEGIN_SRC clojure
  ...
  (->> (periodic-seq (.toInstant
                       (.adjustInto
                         (java.time.LocalTime/of 20 0 0)
                         (java.time.ZonedDateTime/now (java.time.ZoneId/of "America/New_York"))))
                     (java.time.Period/ofDays period-days)))
#+END_SRC
...
#+BEGIN_SRC clojure
  ...
  (->> (periodic-seq (.toInstant
       ...
       (filter (comp #{DayOfWeek/TUESDAY
                       DayOfWeek/FRIDAY}
                     #(.getDayOfWeek %))))

  ;; Week-days
  (->> (periodic-seq ...)
       (remove (comp #{DayOfWeek/SATURDAY
                       DayOfWeek/SUNDAY}
                     #(.getDayOfWeek %))))
  ...
  (->> (periodic-seq (.toInstant
                       (.adjustInto
                         (java.time.LocalTime/of 0 0)
                         (.withDayOfMonth (ZonedDateTime/now (ZoneId/of "America/New_York")) 1)))
                     (Period/ofDays period-days))
...
achieved with something like =(rest (periodic-seq (ZonedDateTime/now) (Duration/ofMinutes
5)))=.
